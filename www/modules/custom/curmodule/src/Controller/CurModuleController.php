 ...
 *  - \Drupal - вказуэ що цей файл належить до ядра Drupal, тому що там ще є
 *   Symfony.
...
/**
 * Простір імен нашого контролера. Подібне на опис вище, але НЕ вказується
 * назва нашого класу.
 */
...
/**
 * Використовуємо клас з друпалу: ControllerBase.
 * Наслідуємось від нього і він зробить за нас всі обовязкові речі,
 *  які роблять контроллери.
 */
...
class CurModuleController extends ControllerBase {
  ...
  /**
   * {@inheritdoc}
   * В Drupal 8 дуже багато будується на renderable arrays и при поверненні
   * даною функцією вмісту сторінки, ми також повинні вернути масив, який
   * спокійно пройде через drupal_render().
   */
  ...
  public function pagesCurModule($pages_name) {
    ...
    $output = [];
    $output['#title'] = $this->t('Детальні курси @pages_name', ['@pages_name' => $pages_name]);
